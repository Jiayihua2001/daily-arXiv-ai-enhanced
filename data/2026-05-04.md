...
TL;DR: FC-guided band selection for MI-BCI outperforms random ablation and reduces CSP fits while maintaining accuracy, with PLV enabling aggressive reduction and wPLI showing robustness.


<details>
  <summary>Details</summary>
Motivation: Reliable MI-BCI requires precise decoding of neural rhythms, but CSP performance depends on spectral range. FBCSP uses predefined bands rather than subject-specific criteria. Thus, we propose FC-guided band selection to improve interpretability and efficiency.

Method: Identify discriminative bands by calculating phase-based connectivity (wPLI, PLV, PLI) across sensorimotor channels. Rank nine bands by effect size of hemispheric coupling differences, prune to top K, then use FBCSP and SVR for classification. Tested on two datasets for K=1..8, benchmarked against standard FBCSP and random ablation.

Result: FC-guided selection can outperform random ablation and achieve near-baseline performance while reducing CSP fits by 22.2% to 77.8%. PLV enables most aggressive reduction prioritizing μ and low-β ranges; wPLI shows inter-session robustness.

Conclusion: FC-guided selection is a principled and interpretable alternative to heuristic filter bank designs.
...
TL;DR: A synthetic molecule exhibits thermoresponsive chiral supramolecular polymorphism controlled by LCST-coupled chirality, enabling stabilization of transient polymorphs via chemical stimuli.


<details>
  <summary>Details</summary>
Motivation: To understand and achieve controlled polymorphic transitions in synthetic chiral supramolecular assemblies, analogous to biological systems.

Method: Designed an unsymmetrical oligo(phenyleneethynylene) derivative with pyridine, amide, and chiral hydrophilic chains; studied self-assembly in water under temperature variation and chemical stimuli.

Result: Three polymorphs formed: short cylinders (AggI), transient double helical fibers (AggII) near LCST, and irregular planar aggregates (AggIII) above LCST; AggII stabilized at RT by co-solvents or metal salts.

Conclusion: LCST-coupled chirality is a powerful strategy for regulating thermoresponsive supramolecular polymorphism, aiding design of adaptive materials.
...
TL;DR: Solid lipid nanoparticles (SLNs) are colloidal carriers with improved stability, controlled release, and scale-up potential, but face challenges like low drug loading and burst release; this review covers fundamentals, production, evaluation, and trends toward nanostructured lipid carriers and Quality-by-Design.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of emulsions, liposomes, and polymeric nanoparticles by offering better physical stability, controlled release, and industrial scalability via high-pressure homogenization.

Method: Review of SLN composition, structure, incorporation models, production methods (e.g., high-pressure homogenization), characterization techniques (particle size, zeta potential, morphology, entrapment efficiency, DSC/XRD, in vitro release, permeation, stability), and route-specific quality requirements.

Result: SLNs enhance delivery of poorly water-soluble drugs, improve topical retention, and protect sensitive molecules; however, issues like limited drug loading, burst release, polymorphic transitions, and drug expulsion persist.

Conclusion: Current trends emphasize shifting from classical SLNs to nanostructured lipid carriers (NLCs), applying Quality-by-Design (QbD), surface engineering, co-delivery, and more predictive in vitro testing to improve robustness and clinical translation.
